	...
	case CMD_SET_INDEX:
		{
			...
			index_bump = StringToUInt(Cmd.Parameters[1].Value);
		}
		return true;
	...
	case CMD_WRITE_INDEX:
		{
			...
			if(size>=1) fputc((index_number>>0)&0xff,File.GetFile());
			if(size>=2) fputc((index_number>>8)&0xff,File.GetFile());
			if(size>=3) fputc((index_number>>16)&0xff,File.GetFile());
			if(size>=4) fputc((index_number>>24)&0xff,File.GetFile());
	...
	case CMD_WRITE_INDEX_FILE:
		{
			...
			if(size>=1) fputc((index_number>>0)&0xff,fp);
			if(size>=2) fputc((index_number>>8)&0xff,fp);
			if(size>=3) fputc((index_number>>16)&0xff,fp);
			if(size>=4) fputc((index_number>>24)&0xff,fp);
